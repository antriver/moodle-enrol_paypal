...
/**
 * paypalenhanced enrolment plugin tests.
 *
...
class enrol_paypalenhanced_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['paypalenhanced'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['paypalenhanced']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('paypalenhanced'));
        $plugin = enrol_get_plugin('paypalenhanced');
        $this->assertInstanceOf('enrol_paypalenhanced_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_paypalenhanced', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $paypalenhancedplugin = enrol_get_plugin('paypalenhanced');

        // Just make sure the sync does not throw any errors when nothing to do.
        $paypalenhancedplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_paypalenhanced_plugin $paypalenhancedplugin  */
        $paypalenhancedplugin = enrol_get_plugin('paypalenhanced');
        ...
        $id = $paypalenhancedplugin->add_instance($course1, $data);
        $instance1  = $DB->get_record('enrol', array('id' => $id));
        $data = array('roleid' => $studentrole->id, 'courseid' => $course2->id);
        $id = $paypalenhancedplugin->add_instance($course2, $data);
        ...
        $paypalenhancedplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $paypalenhancedplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $paypalenhancedplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now - 60);

        $paypalenhancedplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $paypalenhancedplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now - 60 * 60);
        $paypalenhancedplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60 * 60);

        $paypalenhancedplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now - 60 * 60 * 24 * 7, $now - 60);
        $paypalenhancedplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $paypalenhancedplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $paypalenhancedplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));

        $paypalenhancedplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $paypalenhancedplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $paypalenhancedplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
