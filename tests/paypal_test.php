...
/**
 * paypalupgrade enrolment plugin tests.
 *
...
class enrol_paypalupgrade_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['paypalupgrade'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['paypalupgrade']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('paypalupgrade'));
        $plugin = enrol_get_plugin('paypalupgrade');
        $this->assertInstanceOf('enrol_paypalupgrade_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_paypalupgrade', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $paypalupgradeplugin = enrol_get_plugin('paypalupgrade');

        // Just make sure the sync does not throw any errors when nothing to do.
        $paypalupgradeplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_paypalupgrade_plugin $paypalupgradeplugin  */
        $paypalupgradeplugin = enrol_get_plugin('paypalupgrade');
        ...
        $id = $paypalupgradeplugin->add_instance($course1, $data);
        $instance1  = $DB->get_record('enrol', array('id'=>$id));
        $data = array('roleid'=>$studentrole->id, 'courseid'=>$course2->id);
        $id = $paypalupgradeplugin->add_instance($course2, $data);
        ...
        $paypalupgradeplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $paypalupgradeplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $paypalupgradeplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $paypalupgradeplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $paypalupgradeplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $paypalupgradeplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $paypalupgradeplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $paypalupgradeplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $paypalupgradeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $paypalupgradeplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $paypalupgradeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $paypalupgradeplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $paypalupgradeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
