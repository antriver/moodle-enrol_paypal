...
if (! $context = context_course::instance($course->id, IGNORE_MISSING)) {
    message_paypalenhanced_error_to_admin("Not a valid context id", $data);
    die;
}
...
if (strlen($result) > 0) {
    if (strcmp($result, "VERIFIED") == 0) {          // VALID PAYMENT!
        ...
        if ($data->payment_status == "Pending" and $data->pending_reason != "echeck") {
            ...
            $eventdata->component         = 'enrol_paypalenhanced';
            $eventdata->name              = 'paypalenhanced_enrolment';
            ...
            message_paypalenhanced_error_to_admin("Payment pending", $data);
            die;
        }
        ...
        if (core_text::strtolower($data->business) !== core_text::strtolower($plugin->get_config('paypalbusiness'))) {   // Check that the email is the one we want it to be
            message_paypalenhanced_error_to_admin("Business email is {$data->business} (not ".
        ...
        if ($data->payment_gross < $cost) {
            message_paypalenhanced_error_to_admin("Amount paid is not enough ($data->payment_gross < $cost))", $data);
            die;
        ...
        $DB->insert_record("enrol_paypalenhanced", $data, false);
        message_paypalenhanced_error_to_admin("Received an invalid payment notification!! (Fake payment?)", $data);
    }
}
 ...
 */
function enrol_paypalenhanced_ipn_exception_handler($ex) {
    $info = get_exception_info($ex);

    $logerrmsg = "enrol_paypalenhanced IPN exception handler: ".$info->message;
